...
//Changing backer, amount and bar automatically

app.get('/orders', function(request, response) {
    global.db.Orders.findAll().success( function(orders) {
    var total_backers = orders.length;
    response.render("index", {backers: total_backers});

    var amount = 0;
    result.rows.forEach(function(order) {
      amount += order.amount
    });
    response.render("index", {money: amount});
    response.render("index", {percent: amount});
